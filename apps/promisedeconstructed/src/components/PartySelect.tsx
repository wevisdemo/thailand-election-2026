...
const UpdatePartyFromParam = ({
	handleParamPartyUpdate,
}: {
	handleParamPartyUpdate: (party: string | null) => void;
}) => {
	const searchParams = useSearchParams();
	const partyParam = searchParams.get('party');

	useEffect(() => {
		handleParamPartyUpdate(partyParam);
	}, []);
...
export const PartySelect = ({
	...
	const handleSelectChange = useCallback(
		(values: PartySelectChoice['value'][]) => {
			if (
	...
	const handleParamPartyUpdate = useCallback(
		...
		[handleSelectChange, possibleParties],
	);

	return (
		<>
			<Suspense>
				<UpdatePartyFromParam handleParamPartyUpdate={handleParamPartyUpdate} />
			</Suspense>
			<Select.Root
