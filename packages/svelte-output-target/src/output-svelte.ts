...
export const getPathToCorePackageLoader = (
	config: Config,
...
export function generateProxies(
	...
	const buildImports = (dir: string, ext = '') =>
		components
			.map((c) => `import ${fileName(c)} from '${dir}/${fileName(c)}${ext}';`)
			.join('\n');

	const buildExports = () =>
		components.map((c) => `export { ${fileName(c)} };`).join('\n');
